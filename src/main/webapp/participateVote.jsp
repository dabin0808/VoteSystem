            ...
            <div class="col-md-8 participateVote-content-left">
                <div class="row">
                    <form class="form-inline" action="${pageContext.request.contextPath}/participateVoteServlet" method="post" style="text-align: center; margin-top: 15px; margin-bottom: 15px;">
                        <input class="form-control" name="searchKey" value="${participateCondition}" type="text" placeholder="投票主题搜索" />
                        ...
                        <div class="col-md-4">
                            <a href="${pageContext.request.contextPath}/participatePanelServlet?subjectId=${subject.subjectId}" class="thumbnail">
                                ...
                                <div class="caption">
                                    <h4>${subject.title}</h4>
<%--                                    <ul type="circle">--%>
<%--                                        <li><span class="glyphicon glyphicon-star-empty"></span> ${subject.optionSet.get(0).content}</li>--%>
<%--                                        <li><span class="glyphicon glyphicon-star-empty"></span> ${subject.optionSet.get(1).content}</li>--%>
                ...
                <div class="row" style="text-align: center;">
                    <nav aria-label="Page navigation">
                        <ul class="pagination">
                            ...
                            <c:if test="${subjectPageBean.currentPage != 1}">
                                <li>
                                    <a href="${pageContext.request.contextPath}/participateVoteServlet?currentPage=${subjectPageBean.currentPage-1}&rows=6&title=${participateCondition}" aria-label="Previous">
                                        <span aria-hidden="true">&laquo;</span>
                                    </a>
                                </li>
                            </c:if>
                            ...
                            <c:forEach begin="1" end="${subjectPageBean.totalPage}" var="i">
                                <c:if test="${subjectPageBean.currentPage == i}">
                                    <li class="active"><a href="${pageContext.request.contextPath}/participateVoteServlet?currentPage=${i}&rows=6&title=${participateCondition}">${i}</a></li>
                                </c:if>
                                <c:if test="${subjectPageBean.currentPage != i}">
                                    <li><a href="${pageContext.request.contextPath}/participateVoteServlet?currentPage=${i}&rows=6&title=${participateCondition}">${i}</a></li>
                                </c:if>
                            </c:forEach>
                ...
                <c:if test="${participateSub.type == 0}">
                    <form action="${pageContext.request.contextPath}/countOneVotingServlet" method="post" id="oneSelectForm">
                        <table class="table table-bordered table-hover">
                            ...
                            <c:forEach items="${participateSub.optionSet}" var="oneOption" varStatus="s">
                                <c:if test="${orders[0]==oneOption.optionId}">
                                    <tr>
                                        <td><div class="radio" style="color: #2b542c; font-weight: bolder;">
                                            <label>
                                                <input type="radio" checked name="RadioGroup" value="${s.index}" /> ${oneOption.content}
                                            </label>
                                        </div>
                                        </td>
                                    </tr>
                ...
                <c:if test="${participateSub.type == 1}">
                    <form action="${pageContext.request.contextPath}/countMoreVotingServlet" method="post" id="moreSelectForm">
                        <table class="table table-bordered table-hover">
                            ...
                            <c:forEach items="${participateSub.optionSet}" var="moreOption" varStatus="s">
                                <c:choose>
                                    <c:when test="${orders[i]==moreOption.optionId}">
                                        <tr><td><div class="checkbox" style="color: #2b542c; font-weight: bolder;"><label><input checked type="checkbox" name="checkboxGroups" value="${s.index}" /> ${moreOption.content}</label></div></td></tr>
                                        <c:if test="${i<len}" >
                                            <c:set var="i" value="${i+1}"></c:set>
                                        </c:if>
                                    </c:when>
                                    <c:otherwise>
                                        <tr><td><div class="checkbox"><label><input type="checkbox" name="checkboxGroups" value="${s.index}" /> ${moreOption.content}</label></div></td></tr>
                                    </c:otherwise>
                                </c:choose>
                            </c:forEach>
